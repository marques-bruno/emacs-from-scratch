...
The following packages (on ubuntu 20.04) are required for this setup to run smoothly
...
# emacs27 is recommended and must be installed from external repository. instructions are easy to find

#+end_src

The following command should be ran on emacs  after everything is installed:
